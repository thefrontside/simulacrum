...
import { taskable, type StartableTask, type TaskableOperation } from "./taskable.ts";
...
export type ServiceGraphTask<
  S extends Record<string, ServiceDefinition<string, T>>,
  T extends MaybeSimulation,
> = StartableTask<ServiceGraph<S, T>>;

export type ServiceGraphOperation<
  S extends Record<string, ServiceDefinition<string, T>>,
  T extends MaybeSimulation,
> = TaskableOperation<ServiceGraph<S, T>>;

export type ServiceGraphRunner<
  S extends Record<string, ServiceDefinition<string, T>>,
  T extends MaybeSimulation,
> = ((subset?: Array<keyof S>) => ServiceGraphOperation<S, T>) & {
  task(subset?: Array<keyof S>): ServiceGraphTask<S, T>;
};
 ...
 * callers and also exposes `.task(subset?)` for callers that want a
 * promise-first handle whose `.start()` or `await` resolves when the graph is
 * ready and remains running until `.halt()` is called.
 *
 * @param services - a map of service names to definitions
 * @param options - optional configuration: `{ globalData?, watch?, watchDebounce? }`
 * @returns a runner function returning the graph operation with added `.task()` helpers on both the operation and the runner
 */
...
): ServiceGraphRunner<S, T> {
  const runner = ((subset?: Array<keyof S>) => {
    const graph = taskable(
      resource<ServiceGraph<S, T>>(function* (provide) {
    ...
    ) as ServiceGraphOperation<S, T>;

    return graph;
  }) as ServiceGraphRunner<S, T>;

  runner.task = function serviceGraphRunnerTask(subset?: Array<keyof S>) {
    return runner(subset).task();
  };
