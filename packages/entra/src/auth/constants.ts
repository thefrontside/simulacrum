...
// A stable, well-known development RSA keypair. The corresponding public key is
// published at the JWKS endpoint so tokens issued here can be verified locally.
// Do NOT use these keys for anything other than local simulation.
...
// The kid that ties tokens issued here to the public key advertised in the JWKS.
// Entra libraries (MSAL, passport-azure-ad) look up the signing key by this kid.
export const KEY_ID = "NkRGQjI5N0RBNUUwMTYwOEMxQUVGQkJBQTJBODBGNTE2MDA5NDM5RA";
...
const parseKey = (key: string): string => key.split("~~").join("\n");
